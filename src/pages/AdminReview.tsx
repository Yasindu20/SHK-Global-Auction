...
export default function AdminReview() {
  ...
  const fetchListings = async (showRefreshing = false) => {
    ...
    try {
      const res  = await fetch(`${API}/api/listings/all`, { headers: getAdminAuthHeader() });
  ...
  const handleApprove = async (id: string) => {
    try {
      const res = await fetch(`${API}/api/listings/approve/${id}`, {
        method: 'POST',
  ...
  const handleReject = async (id: string) => {
    try {
      const res = await fetch(`${API}/api/listings/reject/${id}`, {
        method: 'POST',
        headers: getAdminAuthHeader(),
      });
