...
export abstract class Crawler {
  ...
  constructor(options: CrawlerOptions = {}) {
    this.options = { delay: 2000, headless: true, ...options };
  }
   ...
   */
  protected gaussianMs(mean: number, stdDev: number): number {
    let u = 0, v = 0;
  ...
  protected async humanDelay(mean = 2000, stdDev = 700): Promise<void> {
    await this.wait(this.gaussianMs(mean, stdDev));
  }
   ...
   */
  protected async createStealthContext(referer?: string): Promise<BrowserContext> {
    ...
    await context.addInitScript(
      ({ hwc, dm }: { hwc: number; dm: number }) => {
        ...
        HTMLCanvasElement.prototype.getContext = function (type: string, ...args: any[]) {
          const ctx = _getCtx.call(this, type, ...args) as CanvasRenderingContext2D | null;
          if (type === '2d' && ctx) {
            const _fill = ctx.fillText.bind(ctx);
            ctx.fillText = (text: string, x: number, y: number, maxWidth?: number) => {
              if (maxWidth !== undefined) _fill(text, x + 0.1, y + 0.1, maxWidth);
              else _fill(text, x + 0.1, y + 0.1);
            };
          }
          return ctx as any;
        };
   ...
   */
  protected async simulateScroll(page: Page, coverage = 0.65): Promise<void> {
    try {
      await page.evaluate(async (cov: number) => {
  ...
  protected async simulateMouseMovement(page: Page): Promise<void> {
    try {
      ...
      for (let i = 0; i < moves; i++) {
        ...
        await page.mouse.move(tx, ty, { steps: Math.floor(Math.random() * 12) + 8 });
        await this.wait(Math.random() * 350 + 80);
      }
    } catch { /* ignore */ }
  }
  ...
  protected async humanType(page: Page, selector: string, text: string): Promise<void> {
    ...
    await this.wait(this.gaussianMs(300, 100));
    for (const ch of text) {
      await page.keyboard.type(ch, { delay: this.gaussianMs(90, 30) });
    }
  }
